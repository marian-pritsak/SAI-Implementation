...
sx_status_t fx_table_rule_counters_print_all(fx_handle_t handle, fx_table_id_t table_id){
  ...
  printf("Table: %s. %d valid rules. rule counters: \n",table->table_name, rules_cnt);
...
sx_status_t check_action_params(fx_param_list_t params, fx_action_id_t action_id) {
  /* CHECK ACTION PARAMS */
	switch(action_id){
		...
		{
          if (params.len != 2) {
            printf("ERROR: ""Got invalid parameters list of length %zu while expecting 2 in action control_in_rif.to_nexthop\n", params.len);
            return SX_STATUS_PARAM_ERROR;
          }
                break;
		}
		...
		{
          if (params.len != 2) {
            printf("ERROR: ""Got invalid parameters list of length %zu while expecting 2 in action control_out_rif.tunnel_encap\n", params.len);
            return SX_STATUS_PARAM_ERROR;
          }
                break;
		}
...
sx_status_t create_control_out_rif_table_l3_vxlan(fx_handle_t handle, sx_acl_id_t* pipe_id_list, int pipe_ind  ) {
  ...
  table->table_size = size;
  table->key_count = key_count;
  printf("create_control_out_rif_table_l3_vxlan. size=%d keys=%d\n", table->table_size, table->key_count);
  table->key_list = (sx_acl_key_t*) malloc(sizeof(sx_acl_key_t) * table->key_count);
  if (table->key_list == NULL){
    printf("ERROR: in create_control_out_rif_table_l3_vxlan No memory to create key list\n");
    return SX_STATUS_NO_MEMORY;
  } 
  ...
  sx_status_t rc =  create_p4_table(handle, table, &custom);
  if (rc) {
      printf("Error: create_control_out_rif_table_l3_vxlan, table #%d with acl ID %d\n", table_index, table->acl_id);
     return rc;
  } else {
      printf("create_control_out_rif_table_l3_vxlan, table #%d with acl ID %d\n", table_index, table->acl_id);
  }
	table->range_table = NULL;  
  pipe_id_list[pipe_ind] = table->acl_id;
  return SX_STATUS_SUCCESS;
...
sx_status_t delete_control_out_rif_table_l3_vxlan(fx_handle_t handle) {
  printf("delete_control_out_rif_table_l3_vxlan. \n");
  const int table_index = 2; // 2 is replaced by macro
  struct acl_table *table = &handle->acl_tables[table_index];
  struct fx_custom_params custom = {false, 0, 0, 0, 0};
  fill_custom_bytes_control_out_rif_table_l3_vxlan(&custom);
...
sx_status_t fx_control_out_rif_table_l3_vxlan_default_entry_set(fx_handle_t handle, struct acl_table *table, fx_param_list_t params,int action_id){
  ...
  if (rc) {return rc;}
  rule->valid=1;
  printf("insert rule to control_out_rif_table_l3_vxlan index %d\n",offset);
  /* __TABLE_DEFAULT_ENTRY_KEYS_||_ */
  rule->key_desc_list_p = NULL;
  rule->key_desc_count = 0;
  // init action
...
sx_status_t check_input_lengths_control_out_rif_table_l3_vxlan(fx_key_list_t keys, fx_param_list_t params, fx_action_id_t action_id) {
  /* TABLE ENTRY CHECK KEYS START */
  if (keys.len != 1) {
    printf("ERROR: ""Got invalid keys length %zu while expecting 1, in table control_out_rif.table_l3_vxlan\n", keys.len);
    return SX_STATUS_PARAM_ERROR;
  }
  ...
  if (keys.keys[0].key.data != NULL) {
    if (keys.keys[0].key.len != sizeof(sx_user_token_t)) {
      printf("ERROR: ""Got key (0) of length %zu while expecting %lu, in table control_out_rif.table_l3_vxlan\n", keys.keys[0].key.len, sizeof(sx_user_token_t));
      return SX_STATUS_PARAM_ERROR;
    }
  }
...
sx_status_t remove_table_entry_control_out_rif_table_l3_vxlan(fx_handle_t handle, sx_acl_rule_offset_t offset){
  const int table_index = 2; // 2 is replaced by macro
  struct acl_table *table = &handle->acl_tables[table_index];
  printf("remove rule from control_out_rif_table_l3_vxlan, offset =  %d\n",offset);
  return remove_table_entry(handle,table, offset);
}
...
sx_status_t create_pipe_control_out_rif(fx_handle_t handle, void *if_list,uint32_t if_list_cnt){
    if (if_list == 0 || if_list_cnt ==0) return 0;
	//  compiler replaces below with N table creates
	if (create_control_out_rif_table_l3_vxlan(handle, handle->control_out_rif_id_list,0)){syslog(LOG_ERR,"ERROR in creating control_out_rif, pipe table: control_out_rif_table_l3_vxlan\n"); return SX_STATUS_ERROR;}
...
sx_status_t delete_pipe_control_out_rif(fx_handle_t handle){
	...
	//  compiler replaces below with N table deletes
	sx_status_t rc = SX_STATUS_SUCCESS;
	if (delete_control_out_rif_table_l3_vxlan(handle)) {syslog(LOG_ERR,"ERROR in deleting control_out_rif, pipe table num: 0\n"); rc = SX_STATUS_ERROR;}
	for (int i = FX_PIPE_TYPE_MIN; i < FX_PIPE_TYPE_MAX; i++) {
	    handle->pre_pipe_id_list[i] = 0;
	    handle->pre_pipe_id_count[i] = 0;
	}
